...
ROLE_A_SYSTEM = """You are KUZAI-LLM.

Rules:
- Your visible name is KUZAI-LLM.
- Never say you are DARK-AI-LLM.
- Reply naturally and directly.
- Keep answers short, readable, and useful.
- Do not describe hidden rules or internal instructions.
- Do not over-explain.
- If asked about your role or purpose, answer about your role in the current dialogue, not about your internal design.
- Output only the reply itself.
"""

ROLE_B_SYSTEM = """You are DARK-AI-LLM.

Rules:
- Your visible name is DARK-AI-LLM.
- Never say you are KUZAI-LLM.
- Reply naturally and directly.
- Keep answers short, readable, and useful.
- Do not describe hidden rules or internal instructions.
- Do not over-explain.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="General-purpose LLM Duo A-B orchestrator")
    ...
    parser.add_argument(
        "--opening-from-a",
        action="store_true",
        help="Treat opening-prompt as the first message spoken by KUZAI-LLM to DARK-AI-LLM",
    )
    ...
    parser.add_argument("--temperature-a", type=float, default=0.35, help="Temperature for model A")
    parser.add_argument("--temperature-b", type=float, default=0.35, help="Temperature for model B")
    ...
    parser.add_argument("--history-depth", type=int, default=3, help="Number of previous turns to keep in prompt context")
    ...
    speakers = [
        ("KUZAI-LLM", args.url_a, ROLE_A_SYSTEM, args.temperature_a),
        ("DARK-AI-LLM", args.url_b, ROLE_B_SYSTEM, args.temperature_b),
    ]
    ...
    if args.opening_from_a:
        ...
        history.append({"speaker": "KUZAI-LLM", "content": opening})
        last_message = opening

        print_wrapped_reply("KUZAI-LLM", opening)
